...
func (n *cachedNode) obj(hash common.ExtHash) node {
	if node, ok := n.node.(rawNode); ok {
		return mustDecodeNode(hash.Bytes(), node)
	}
	return expandNode(hash.Bytes(), n.node)
}
...
func (db *Database) InsertBlob(hash common.ExtHash, blob []byte) {
	db.lock.Lock()
...
func (db *Database) getCachedNode(hash common.ExtHash) []byte {
	if db.trieNodeCache != nil {
		if enc := db.trieNodeCache.Get(hash.Bytes()); enc != nil {
...
func (db *Database) node(hash common.ExtHash) (n node, fromDB bool) {
	...
	if enc := db.getCachedNode(hash); enc != nil {
		if dec, err := decodeNode(hash.Bytes(), enc); err == nil {
			return dec, false
		} else {
	...
	db.setCachedNode(hash.Bytes(), enc)
	return mustDecodeNode(hash.Bytes(), enc), true
}
...
func (db *Database) Cap(limit common.StorageSize) error {
	...
	for size > limit && oldest != (common.ExtHash{}) {
		...
		enc := node.rlp()
		if err := database.PutAndWriteBatchesOverThreshold(batch, oldest.Bytes(), enc); err != nil {
			db.lock.RUnlock()
			return err
		}

		if db.trieNodeCache != nil {
			db.trieNodeCache.Set(oldest.Bytes(), enc)
		}
...
func (db *Database) writeBatchNodes(node common.ExtHash) error {
	...
	if db.trieNodeCache != nil {
		db.trieNodeCache.Set(node.Bytes(), enc)
	}
...
func (db *Database) commit(hash common.ExtHash, resultCh chan<- commitResult) {
	...
	enc := node.rlp()
	resultCh <- commitResult{hash.Bytes(), enc}

	if db.trieNodeCache != nil {
		db.trieNodeCache.Set(hash.Bytes(), enc)
	}
}
