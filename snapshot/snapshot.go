...
func diffToDisk(bottom *diffLayer) *diskLayer {
	...
	for hash := range bottom.destructSet {
		...
		if base.genMarker != nil && bytes.Compare(hash.Bytes(), base.genMarker) > 0 {
			continue
		}
		// Remove all storage slots
		batch.DeleteAccountSnapshot(hash)
		base.cache.Set(hash.Bytes(), nil)
	...
	for hash, data := range bottom.accountData {
		...
		base.cache.Set(hash.Bytes(), data)
	...
	for accountHash, storage := range bottom.storageData {
		// Skip any account not covered yet by the snapshot
		if base.genMarker != nil && bytes.Compare(accountHash.Bytes(), base.genMarker) > 0 {
			continue
		}
		// Generation might be mid-account, track that case too
		midAccount := base.genMarker != nil && bytes.Equal(accountHash.Bytes(), base.genMarker[:common.ExtHashLength])

		for storageHash, data := range storage {
			// Skip any slot not covered yet by the snapshot
			if midAccount && bytes.Compare(storageHash.Bytes(), base.genMarker[common.ExtHashLength:]) > 0 {
				continue
			}
			if len(data) > 0 {
				batch.WriteStorageSnapshot(accountHash, storageHash, data)
				base.cache.Set(append(accountHash.Bytes(), storageHash.Bytes()...), data)
				snapshotCleanStorageWriteMeter.Mark(int64(len(data)))
			} else {
				batch.DeleteStorageSnapshot(accountHash, storageHash)
				base.cache.Set(append(accountHash.Bytes(), storageHash.Bytes()...), nil)
			}
			snapshotFlushStorageItemMeter.Mark(1)
...
func (t *Tree) Rebuild(root common.ExtHash) {
	...
	t.diskdb.DeleteSnapshotRecoveryNumber()
	t.diskdb.DeleteSnapshotDisabled()
