...
//
// This file is derived from core/state/snapshot/iterator_fast.go (2021/10/21).
...
func (its weightedIterators) Less(i, j int) bool {
	...
	switch bytes.Compare(hashI.Bytes(), hashJ.Bytes()) {
	case -1:
		return true
	case 1:
		return false
...
func (fi *fastIterator) init() {
	...
	var positioned = make(map[common.ExtHash]int)
...
func (fi *fastIterator) next(idx int) bool {
	...
	if diff := bytes.Compare(curHash.Bytes(), nextHash.Bytes()); diff < 0 {
