...
//
// This file is derived from core/state/snapshot/journal.go (2021/10/21).
...
func loadAndParseJournal(db database.DBManager, base *diskLayer) (snapshot, journalGenerator, error) {
	// Retrieve the disk layer generator. It must exist, no matter the
	// snapshot is fully generated or not. Otherwise the entire disk
	// layer is invalid.
	generatorBlob := db.ReadSnapshotGenerator()
	...
	journal := db.ReadSnapshotJournal()
	if len(journal) == 0 {
		logger.Warn("Loaded snapshot journal", "diskroot", base.root, "diffs", "missing")
	...
	if !bytes.Equal(root.Bytes(), base.root.Bytes()) {
		logger.Warn("Loaded snapshot journal", "diskroot", base.root, "diffs", "unmatched")
		return base, generator, nil
	}
	...
	logger.Debug("Loaded snapshot journal", "diskroot", base.root, "diffhead", snapshot.Root())
	return snapshot, generator, nil
}

// loadSnapshot loads a pre-existing state snapshot backed by a key-value store.
func loadSnapshot(diskdb database.DBManager, triedb *statedb.Database, cache int, root common.ExtHash, recovery bool) (snapshot, bool, error) {
	// If snapshotting is disabled (initial sync in progress), don't do anything,
	// wait for the chain to permit us to do something meaningful
	if diskdb.ReadSnapshotDisabled() {
		return nil, true, nil
	}
	// Retrieve the block number and hash of the snapshot, failing if no snapshot
	// is present in the database (or crashed mid-update).
	baseRoot := diskdb.ReadSnapshotRoot()
	//if baseRoot == (common.InitExtHash()) {
	if baseRoot.ToHash() == (common.Hash{}) {
	...
	if err != nil {
		logger.Warn("Failed to load new-format journal", "error", err)
...
func loadDiffLayer(parent snapshot, r *rlp.Stream) (snapshot, error) {
	...
	destructSet := make(map[common.ExtHash]struct{})
	...
	accountData := make(map[common.ExtHash][]byte)
	...
	storageData := make(map[common.ExtHash]map[common.ExtHash][]byte)
	for _, entry := range storage {
		slots := make(map[common.ExtHash][]byte)
...
func (dl *diskLayer) Journal(buffer *bytes.Buffer) (common.ExtHash, error) {
	...
	if dl.stale {
		return common.InitExtHash(), ErrSnapshotStale
	}
	// Ensure the generator stats is written even if none was ran this cycle
	journalProgress(dl.diskdb.GetSnapshotDB(), dl.genMarker, stats)

	logger.Debug("Journalled disk layer", "root", dl.root)
	return dl.root, nil
}

// Journal writes the memory layer contents into a buffer to be stored in the
// database as the snapshot journal.
func (dl *diffLayer) Journal(buffer *bytes.Buffer) (common.ExtHash, error) {
	...
	for hash, slots := range dl.storageData {
		keys := make([]common.ExtHash, 0, len(slots))
	...
	logger.Debug("Journalled diff layer", "root", dl.root, "parent", dl.parent.Root())
	return base, nil
}
